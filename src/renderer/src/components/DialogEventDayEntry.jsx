...
import { calculateTime } from './calendarcomps/dayview/functions/functionHandler';
import { MdAddAlert, MdAlarm, MdClose, MdColorize, MdEvent, MdNote, MdNoteAdd, MdPublic, MdPublicOff, MdTimelapse, MdTimer } from 'react-icons/md';
import Switch from './Switch';
import { isDate } from 'date-fns';
import dayjs from 'dayjs';
registerLocale('de-DE', de) 
const DialogEventDayEntry = ({show, close, title, message, callbackBtn2}) => { 
    ...
    const [color,setcolor]=useState("#ff72eaFF") 
    const closer=(e)=>{
        ...
        {show && (
                <div  
                  className="fixed inset-0 z-10  dark:bg-gray-600 bg-black dark:bg-opacity-50 bg-opacity-40 flex justify-center items-center"
                   onClick={closer}
                >
                  <div aria-label='Ditab' className="min-w-[30%] max-w-[32%] flex z-40 flex-col ring-1 ring-[#0c101b] dark:ring-[#1b243b] rounded-sm">
                  <div id="titlebar" className={'w-1/2 dark:bg-[#1b243b] bg-[#0c101b] flex flex-row items-center justify-between'} > 
                      ...
                      <div className='w-full col-span-1 h-full flex flex-row items-center justify-start'><a className='mr-2 text-sm'>START</a> <MdTimelapse className='text-xl mr-2' /></div>
                      ...
                      <div className='w-full col-span-1 h-full flex flex-row items-center justify-start'><a className='mr-2.5 text-sm'>ENDE</a> <MdTimelapse className='text-xl mr-2' /></div>
                      <div className='col-span-3 w-full'>
                        <DatePicker 
                                        placeholderText="TT.MM.YYYY"
                                        locale={de}
                        ...
                        <div className='w-[70%] h-10 flex flex-row items-center justify-start'>
                            {
                              ...
                              <div className='w-full col-span-1 h-full flex flex-row items-center justify-center'> <MdAlarm className='text-xl mr-2' /></div>
                      ...
                      <div className='w-full flex flex-row items-start justify-start dark:bg-gray-900 bg-white px-6 py-2 dark:text-white text-black text-sm font-[Arial]'>
                        ...
                        <label className='  w-5/6 flex flex-col items-center justify-center relative'> 
                            ...
                            {ispublic?<MdPublic className='absolute inset left-4 text-lg top-[0.55rem] dark:text-blue-200/60 text-gray-900/40 ' />:<MdPublicOff className='absolute inset left-4 text-lg top-[0.55rem] dark:text-blue-200/60 text-gray-900/40 ' />} 
                            <MdClose onClick={()=>'handleFilterChange("Betrefftxt", "")'} className={'absolute hidden cursor-pointer inset right-3 text-2xl top-[0.1rem] text-gray-500 hover:text-gray-400'}  />
                        </label>
                            </div>
                            </div>
                      ...
                      <div className="w-full flex flex-row items-center justify-end dark:bg-gray-900 bg-white px-6 py-2  dark:text-white text-black text-sm font-[Arial]">
                      {
                        ...
                        <button onClick={(e)=>callbackBtn2([betreff,start,end,notice,remind,remindon,color,ispublic])} className={`px-4 py-2 mb-2 rounded-sm dark:bg-gray-800 bg-gray-300/60 dark:hover:bg-gray-700 hover:bg-gray-100 outline-none ring-1 dark:ring-gray-700 ring-gray-400/80 dark:text-gray-300 text-gray-800`}>Bestätigen</button>:''
                      }
                      </div> 
                </div>
                </div>
