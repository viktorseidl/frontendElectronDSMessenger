...
const RowMessageOut = ({ item }) => {
  ...
  return (
    <Link
      to={'/message/' + item.ID}
      state={item}
      className="w-full dark:bg-gray-900 bg-white dark:hover:bg-orange-300/20 hover:bg-blue-500/10 cursor-pointer grid grid-cols-12 items-start justify-items-start"
    >
      <div className="w-full flex flex-row items-center justify-center py-3">
        <div className="w-12 aspect-square dark:bg-blue-100 bg-blue-300 text-gray-800 rounded-full flex flex-col items-center justify-center">
          <b className="text-sm uppercase">
            {User.Name.includes('+')
              ? User.Name.replace(/\+/g, '')[0] + User.Name.replace(/\+/g, '')[1]
      ...
      <div className="w-full col-span-11 px-8  py-2 pl-0 flex flex-col items-start justify-start ">
        <div className="w-full flex flex-row items-start justify-between">
          <b className="w-full flex flex-row truncate">
            <span className="w-20 dark:text-gray-400 font-normal text-sm mr-1">Gesendet an:</span>
            <span className="w-6 aspect-square dark:text-gray-400 text-black/70 mr-4 flex flex-col items-center justify-center ">
              <FaShareFromSquare className="inline" />
            </span>
            <span className="w-[85%] flex flex-wrap gap-1 py-1">
              {item.Sendername.map((it, index) => (
                <div
                  key={it + index}
                  title={'Gesendet an ' + it}
                  className="min-w-16 max-w-20  text-xs dark:bg-blue-100 bg-gray-100 text-gray-800 rounded mr-1 flex flex-col items-center justify-center ring-1 dark:ring-gray-700 ring-gray-300"
                >
          ...
          <div className=" w-32 text-sm dark:text-white text-black font-sans font-semibold">
            {item.FormattedDate}
          </div>
        </div>
        <div className="w-full flex flex-row items-start justify-between">
          <div className="text-[13px] dark:text-gray-300 text-gray-600 font-[arial] w-[90%] flex flex-row items-center justify-start">
            ...
            <p className="pl-4 truncate">
              <b className="dark:text-white text-gray-700 text-[16px] mr-4"> {item.Betreff}</b>
              {item.Nachricht}
            </p>
          </div>
          <div className="text-sm flex flex-row items-start justify-start"></div>
        </div>
      </div>
    </Link>
  )
