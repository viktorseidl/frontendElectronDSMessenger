...
const NewMessageTab = () => {
    ...
    fils.forEach((file) => {
      if (!files.some((item) => item.name === file.name)) {
        ...
        if (filescount > 10000000) {
          ...
          setFiles((previousItem) =>
            previousItem.some((item) => item.name === file.name)
              ? previousItem
              : [...previousItem, file]
          )
        }
  ...
  const handleDialogOpenerField = (e) => {
    if (e.target.getAttribute('aria-label') != 'userstab') {
  ...
  useEffect(() => {
    if (locationData.state === null) {
      ...
      setNachricht(
        '\n\n---------------------------------------------------------------------------------------------------------------------------\nDatum: ' +
          (locationData.state.Datum.split(' ')[0].split('-')[2] +
            '.' +
            locationData.state.Datum.split(' ')[0].split('-')[1] +
            '.' +
            locationData.state.Datum.split(' ')[0].split('-')[0] +
            ' ' +
            locationData.state.Datum.split(' ')[1].split('.')[0]) +
          '\nAbsender: ' +
          locationData.state.Sender +
          '\nBetreff: ' +
          locationData.state.Betreff +
          '\nNachricht: \n\n' +
    ...
    <div className=" flex-grow max-h-full overflow-auto flex flex-col items-start justify-start w-full py-4">
      <div className=" w-full h-20 flex flex-row items-center justify-end -mt-2">
        ...
        <div className="w-full px-4 flex flex-row items-center justify-start gap-x-4">
          <label className="  w-3/4 flex flex-col items-center justify-center relative">
            <div
              onClick={(e) => handleDialogOpenerField(e)}
            ...
            >
              {formattedAddresses.length > 0
                ? formattedAddresses.map((item, index) => (
                    <div
                      aria-label="userstab"
                      key={item + index + 'addresser'}
                      onClick={(e) => e.stopPropagation()}
                      className="dark:bg-lime-700 dark:hover:bg-lime-600 ring-1 dark:ring-gray-800 ring-gray-400 bg-blue-200 rounded px-2 py-1 w-auto dark:text-white text-black shadow-lg shadow-[rgba(0,0,0,0.12)] flex flex-row items-center justify-start "
                    >
                      <a>{item}</a>
                      <MdClose onClick={() => removeItem(item)} className="ml-2 cursor-pointer" />
                    </div>
                  ))
            ...
            <MdClose
              onClick={() => setaddressant([])}
              className={
                'absolute cursor-pointer inset right-3 text-2xl top-[0.4rem] text-gray-500 hover:text-gray-400'
              }
              style={{ display: formattedAddresses.length > 0 ? 'block' : 'none' }}
            />
          </label>
          <div className="w-auto h-full flex flex-row items-center justify-start ">
            <button
              ...
              className="py-2 rounded outline-none ring-1 dark:ring-gray-700 ring-gray-700/30 px-2 flex flex-col items-center justify-center dark:bg-gray-800 dark:hover:bg-gray-700 bg-gray-100 hover:bg-gray-200 dark:text-white text-gray-800"
            >
              <MdGroups3 className="inline" />
            </button>
          </div>
        </div>
        <div className="w-20 relative group p-5 h-full flex flex-col items-center justify-center">
          <div
            style={{ animation: 'fadeIn 0.1s ease-in forwards' }}
            className="absolute w-40 inset right-3 group-hover:block dark:hover:bg-gray-800 hover:bg-gray-200 shadow-lg ring-1 dark:ring-gray-700 ring-gray-300 hidden text-2xl top-10 rounded dark:bg-gray-900 bg-white  "
          >
            <div
              onClick={() => {
              ...
              className="w-full cursor-pointer px-2 flex flex-row items-center justify-center text-sm dark:text-gray-200 text-gray-800 py-2"
            >
        ...
        <div className="w-2/3 px-4 ">
          <label className="  w-full flex flex-col items-center justify-center relative">
            ...
            <MdClose
              ...
              style={{ display: betreff.length > 0 ? 'block' : 'none' }}
            />
          </label>
        </div>
        <div className="w-auto px-4 flex flex-row items-start justify-start gap-x-4 ">
          {priority == 0 ? (
            <button
              ...
              title="Als wichtig kennzeichnen"
              className="py-2 rounded outline-none ring-1 dark:ring-gray-700 ring-gray-700/30 px-2 flex flex-col items-center justify-center dark:bg-gray-800 dark:hover:bg-gray-700 bg-gray-100 hover:bg-gray-200 dark:text-white text-gray-800"
            >
              <MdPriorityHigh className="inline" />
            </button>
          ) : (
            <button
              onClick={() => setpriority(0)}
              title="Kennzeichnung aufheben"
              className="py-2 rounded outline-none ring-1 dark:ring-red-700 ring-red-700 px-2 flex flex-col items-center justify-center dark:bg-red-800 dark:hover:bg-red-700 bg-red-700 hover:bg-red-600 dark:text-white text-white"
            >
              <MdPriorityHigh className="inline" />
            </button>
          )}
      ...
      {files.length > 0 ? (
        <div className="text-white mb-4 min-h-14 max-h-32 w-full overflow-auto dark:scrollbar-thumb-gray-800 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-track-gray-600 scrollbar-track-gray-200 flex flex-wrap items-center justify-start  gap-2 ring-1 dark:ring-gray-800 shadow-inner shadow-[rgba(0,0,0,0.3)] ring-gray-300 p-2">
          {files.map((item, index) => (
            <div
              key={item.name + index}
              className="w-60 flex flex-col items-start justify-start dark:bg-blue-900/90 bg-gray-200 dark:hover:bg-blue-900 hover:bg-gray-300 ring-1 dark:ring-blue-800 ring-gray-500/40 rounded p-2 dark:text-white text-black"
            >
              <div className="w-full flex flex-row items-center justify-start gap-x-4 ">
                <div className="w-auto select-none flex flex-col items-center justify-center">
                  {returnIconType(item.type)}
                </div>
                <div className="w-24 select-none flex flex-row items-center justify-start text-xs truncate  text-[10px]">
                  {returnSizeValue(item.size)}
                </div>
      ...
      <div className="w-full dark:bg-gray-900 bg-white h-full  overflow-auto dark:scrollbar-thumb-gray-800 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-track-gray-600 scrollbar-track-gray-200 flex flex-col items-start justify-start ring-1 dark:ring-gray-700 ring-gray-400 shadow-inner shadow-[rgba(0,0,0,0.3)]  ">
        <textarea
          ...
          className="w-full h-full p-4 dark:placeholder:text-blue-200/60 bg-[#edeae9] dark:text-white  placeholder:text-gray-500  text-gray-800 dark:bg-transparent outline-none shadow-gray-700/25    resize-none text-xl overflow-auto dark:scrollbar-thumb-gray-800 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-track-gray-600 scrollbar-track-gray-200"
      ...
      <Dialog
        ...
        title={'Information'}
        message={
          'Jede Nachricht kann nur Anhänge mit einer maximalen Größe von 10 MB versendet werden. Daher werden nur die ausgewählten Dateien berücksichtigt.'
        }
        cancelBtn={true}
      />
      ...
      {betreff.trim().length > 0 && nachricht.trim().length > 0 && addressant.length > 0 ? (
        <div className="fixed bottom-20 right-16">
          <div
            ...
            className="group cursor-pointer  dark:bg-blue-600 dark:hover:bg-blue-700 bg-blue-600 hover:bg-blue-700 hoverbtnsendbtn p-3 rounded-2xl shadow-lg flex flex-col items-center justify-center dark:shadow-[rgba(0,0,0,0.4)] shadow-[rgba(0,0,0,0.4)] text-white text-3xl"
          >
            <BsFillSendFill className="group-hover:size-[124%]" />
          </div>
        </div>
      ) : (
        ''
