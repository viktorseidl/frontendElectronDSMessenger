...
const DatenbankConfigureSuccess = () => {
  ...
  const returnInitial=(N)=>{ 
    return(<><div className='w-full flex flex-row items-center justify-start'><MdArrowForward className='inline mr-4' /><span className='mr-4'>Tabelle "{N}" wird eingerichtet</span> <Loader Fill={' fill-blue-600 '} Background={' text-blue-300 '} Width={' w-4 '}  /></div></>)
  }
  const returnfinal=(N)=>{ 
    return(<><div className='w-full flex flex-row items-center justify-start'><MdArrowForward className='inline mr-4' /><span className='mr-4'>Tabelle "{N}" wird eingerichtet</span> <MdDone className='inline' /></div></>)
  }
  useEffect(()=>{
    setTimeout(()=>{
      setthree(true)
  ...
  return (
    ...
    <div className='w-full flex flex-col items-center justify-start mt-8'> 
        ...
        {
          four?
          <div className='w-full mt-8 flex flex-col items-center text-white justify-center dark:bg-lime-700 bg-lime-600/40 rounded-sm py-2'>
