...
const DatabaseScreen = () => {
  ...
  const handleCheckConnection = async () => {
    seterr(false)
    if (
      host.trim().toString().length > 3 &&
      localhost.trim().toString().length > 5 &&
      dbname.trim().toString().length > 3 &&
      user.trim().toString().length >= 2
    ) {
      setshowloader(9)
      try {
        const hostcheck = await useFetchAuthAll(
          'http://' + localhost + '/electronbackend/index.php?path=testHostConnection',
          'ssdsdsd',
          'GET',
          null,
          null
  ...
  const checkDatabaseCredentials = async () => {
    const check = await useFetchAuthAll(
      'http://' + localhost + '/electronbackend/index.php?path=testDBConnection',
      'ssdsdsd',
      'POST',
      {
        host: host,
  ...
  const CheckAndCreateTables = async () => {
    setshowloader(7)
    const check = await useFetchAuthAll(
      'http://' + localhost + '/electronbackend/index.php?path=testDBTables',
      'ssdsdsd',
      'POST',
      {
        host: host,
  ...
  const checkCookie = async () => {
    if (window.localStorage.getItem('dbConfig')) {
      const Configexists = await useFetchAuthAll(
        'http://' + localhost + '/electronbackend/index.php?path=checkConfigFileConnector',
        'ssdsdsd',
        'GET',
        null,
        null
    ...
    const cookie = {
      ...
      value: util.encode64(
        JSON.stringify({
          ...
          dbnamepflege: dbnamepflege.toString(),
          user: user.toString(),
          password: password.toString(),
          localhost: localhost.toString()
        })
      ),
  ...
  return (
    <div
      className={
        'pt-8 px-1 w-screen dark:text-gray-200 text-gray-800 dark:bg-gray-950 bg-stone-100 tabsgrounddark flex flex-col items-start justify-start h-screen '
      }
    >
      ...
      <div className="w-full h-full z-10  flex flex-col items-center justify-center">
        {showloader == 1 ? (
          ...
          <div className="w-full flex flex-col items-center justify-center h-full ">
            <div
              className={
                ' w-2/6 border animate-slide-inleft dark:border-gray-800 border-gray-400 shadow-lg dark:shadow-blue-900/80 shadow-gray-500/80 dark:bg-[#0a0e16] bg-gray-200 rounded pt-10 flex flex-col items-center justify-center  -mt-20'
              }
            >
              <div className="w-full mb-6  text-3xl font-[arial] text-center">
              ...
              <label className="w-full flex flex-col items-center justify-start">
                ...
                <input
                  onKeyDown={(event) => {
                    event.key === 'Enter' ? handleCheckConnection() : ''
                  }}
                  type="text"
              ...
              <div className="w-5/6 flex mb-8 flex-row items-end justify-end">
                <button
                  ...
                  className="py-1 px-4 text-sm dark:text-white text-black mt-14 rounded-sm dark:bg-[#1f273f] bg-gray-300 dark:hover:bg-gray-700 outline-none hover:bg-gray-400/60 shadow-inner dark:shadow-[rgba(0,0,0,0.1)] shadow-[rgba(255,255,255,0.1)] ring-1 dark:ring-gray-600 ring-gray-400"
                >
                  Speichern
                </button>
              </div>
