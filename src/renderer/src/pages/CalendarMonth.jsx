...
import { MdCalendarViewMonth, MdFilterList } from 'react-icons/md'
...
import { getGermanHolidays } from '../components/calendarcomps/monthview/functions/functionHandler'
...
const CalendarMonth = () => {
  ...
  const getMonthDays = () => {
    ...
    const startOfCurrentMonth = startOfMonth(b)
    const endOfCurrentMonth = endOfMonth(b)
  ...
  const getFeiertage = () => {
    ...
    Feiertage = [...Feiertage].filter((e) => getMonthDays().includes(e.ddate))
    return Feiertage
  }
  ...
  const getEventsDB = async () => {
    ...
    const query = await useFetchAuthAll(
      'http://' +
        apache +
        '/electronbackend/index.php?path=getMonthEvents&a=' +
        util.encode64(
          User.Name +
            ':' +
  ...
  const handleDrop = async (item, slot) => {
    ...
    const newSlotDate = slot
    console.log(slot)
    const newSlotTime = item.realtimestart
