...
const LogScreen = () => {
  ...
  const loginUser = async () => {
    if (type && user.trim().length > 0 && password.trim().length > 0) {
      ...
      const check = await useFetchAuthAll(
        'http://' + apache + '/electronbackend/index.php?path=checkCredentials',
        'ssdsdsd',
        'POST',
        {
          dbtype: type,
      ...
      if (check[0]?.length > 0) {
        ...
        window.sessionStorage.setItem('userWohnbereiche', util.encode64(JSON.stringify(check[2])))
  ...
  return (
    <div
      className={
        'pt-8 px-1 w-screen dark:text-gray-200 text-gray-800 dark:bg-gray-950 bg-stone-100 flex flex-col items-start justify-start h-screen tabsgrounddark '
      }
    >
          ...
          >
            ...
            <label className="w-full mt-4 flex flex-col items-center justify-start relative">
              ...
              {showpass ? (
                <FaRegEye
                  ...
                  className="absolute inset right-[12%] top-[1.2rem] text-gray-500 hover:text-gray-400 cursor-pointer"
                />
              ) : (
                <FaRegEyeSlash
                  onClick={() => setshowpass(true)}
                  className="cursor-pointer absolute inset right-[12%] top-[1.2rem] text-gray-500 hover:text-gray-400"
                />
              )}
            </label>
            <div className="w-5/6 flex mb-8 flex-row items-end justify-end">
              <button
                ref={_lockbtn}
                onClick={() => loginUser()}
                className="py-2 px-4 text-sm dark:text-white text-black font-bold mt-14 rounded dark:bg-[#1f273f] bg-gray-300 dark:hover:bg-gray-700 hover:bg-gray-400/60 shadow-inner shadow-[rgba(255,255,255,0.1)] ring-1 dark:ring-gray-800 ring-gray-400 flex flex-row items-center justify-center"
              >
                {logloader == false ? (
                  'Anmelden'
                ) : (
                  <>
                    <Loader Fill={' fill-blue-600 '} Background={' text-blue-300 '} Width={4} />{' '}
                    <a className="ml-2 font-normal">...Daten werden geprüft</a>
                  </>
                )}
              </button>
            </div>
